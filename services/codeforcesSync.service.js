...
class CodeforcesSyncService {
  ...
  async fetchProblemStatement(contestId, index) {
    ...
    try {
      ...
      // Fallback: возвращаем ссылку на задачу вместо полного описания
      console.warn(`Using fallback description for ${contestId}${index}`);
      return {
        description: `Это задача с Codeforces. Полное условие доступно по ссылке ниже.\n\n**Важно:** Решите задачу на сайте Codeforces и скопируйте правильное решение сюда для проверки.`,
        samples: [],
        url
      };
    }
